...
3. 실패 사례와 evaluator feedback을 바탕으로 OCR 규칙 후보를 생성한다.
...
- `src/glm_ocr_prompt_optimization/ocr_client.py`
  - 실제 OCR 모델 호출 계층이다.
  - 기본값은 OpenAI 호환 API 형태로 GLM-OCR를 호출한다.
...
- `src/glm_ocr_prompt_optimization/optimizer.py`
  - 실패 사례를 요약하고 Arize Prompt Learning SDK 출력에서 OCR 규칙 후보를 추출한다.
  - 반복, 번역, 숫자 필드, 줄바꿈, markdown leakage 같은 OCR 민감 규칙을 묶음 단위로 탐색한다.
...
기본 candidate strategy는 `ocr-rules`이며, 자유형 prompt rewrite가 아니라 OCR 규칙 묶음을 비교하도록 설계돼 있다.

## 2.3 smoke와 benchmark의 역할 구분

- `./scripts/run_vllm_smoke_experiment.sh`
  - 1라운드만 돌린다.
  - 목적은 연결, tracing, guardrail, artifact 형식 확인이다.
  - 성능 향상 결론을 내리는 용도로 쓰지 않는다.
- `./scripts/run_vllm_benchmark_experiment.sh`
  - 3라운드와 더 많은 후보를 사용한다.
  - 목적은 실제 prompt optimization 성능 비교다.
  - 보고서와 채택 판단은 이 경로를 기준으로 한다.
...
### 11.1 vLLM 서빙용 docker compose

루트의 [`docker-compose.yml`](/home/primi/workspace/GLM-OCR-prompt-optimization/docker-compose.yml)과 [`Dockerfile.vllm`](/home/primi/workspace/GLM-OCR-prompt-optimization/Dockerfile.vllm)은 Hugging Face에서 모델을 직접 내려받아 `vllm`으로 서빙하는 기본 구성을 제공한다.

이 구성은 `vllm/vllm-openai:v0.17.1`를 베이스로 쓰되, GLM-OCR를 인식하지 못하는 기본 `transformers 4.57.6` 대신 `transformers==5.1.0`를 이미지에 미리 bake한다. 따라서 재실행할 때마다 `pip install`을 다시 하지 않는다.

자주 같이 쓰는 환경 변수:

- `HF_TOKEN`
  - 비공개 또는 인증이 필요한 모델일 때 사용
- `HF_HOME`
  - Hugging Face 캐시 경로
...
```bash
HF_TOKEN=$(grep '^export HF_TOKEN=' ~/.bashrc | cut -d= -f2- | tr -d '"') \
