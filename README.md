...
- `src/glm_ocr_prompt_optimization/metrics.py`
  - CER, normalized CER, token F1, non-Korean/repetition/garbage penalty를 계산한다.
...
validation에서는 단순히 점수만 조금 높다고 바로 최종 프롬프트를 채택하지 않는다.

`experiment.py`의 규칙은 대체로 아래와 같다.

- optimized prompt가 너무 길면 탈락
- CER가 좋아지고 stability 지표도 개선되면 채택
- 점수가 사실상 비기면 더 짧은 프롬프트 채택
- 그 외에는 baseline 유지

즉, 이 프로젝트는 “조금 더 공격적인 프롬프트”보다 “실제로 더 안정적인 프롬프트”를 채택하려고 설계되어 있다.
...
- `token F1`
  - 토큰 단위 겹침 정도
- `base_score`
  - normalized CER와 token F1을 합친 기본 점수
- `penalties`
  - 비정상 출력에 대한 감점

현재 penalty는 주로 아래를 본다.

- non-Korean/CJK contamination
  - 한국어가 한자 등으로 오염되는 현상
- repetition
  - 반복 n-gram, 이상한 문자 반복
- empty_or_garbage
...
최종 비교에는 `mean_total_score`가 주로 쓰이지만, 보고서에서는 CER, token F1, penalty rate를 함께 보는 것이 맞다.
